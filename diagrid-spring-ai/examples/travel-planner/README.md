...
> profiles layers in the `io.diagrid:diagrid-spring-ai-*` starters with **no application-code changes**
...
AI out of the box, and layers in the `diagrid-spring-ai` starters — durability, memory, and registry —
via Spring profiles.
...
in-process; under the `memory` profile the `diagrid-spring-ai-memory` starter backs `ChatMemory` with a
...
The `diagrid-spring-ai` durability layer is instrumented with Micrometer, but it **no-ops** unless the
...
> **App and sidecar in one trace (needs the observed workflow client).** diagrid-spring-ai's starter
...
- **`conversation`** — the `diagrid-spring-ai-conversation` ChatModel: LLM traffic leaves the app as a
...
```
travel-planner/
├── pom.xml                 Spring Boot 4.0.5 parent, Java 21, spring-ai 2.0.0 + the 3 diagrid-spring-ai starters; `-Ptracing` profile
